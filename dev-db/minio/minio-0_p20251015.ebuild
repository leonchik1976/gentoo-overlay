...
# Upstream does not ship a vendor/ directory for this release, so the
# module list below is generated from the release's own go.sum with:
...
# tarball ("you need to also create a dependency tarball and host it
# somewhere, for example in your dev space"), which this personal
# overlay has no hosting for. Kept anyway because it remains fully
# offline-reproducible via Manifest-pinned distfiles, but as an
# accepted, deprecated packaging choice, not a resolved or clean one.
EGO_SUM=(
	"aead.dev/mem v0.2.0"
...
# module dependencies resolved from go.sum (verified against each
# module's own LICENSE file in the local module cache).
LICENSE="AGPL-3 Apache-2.0 BSD BSD-2 CC0-1.0 EPL-2.0 ISC LGPL-3 MIT MPL-2.0"
SLOT="0"
