...
src_prepare() {
	...
	# classic-level also appears in the CUA runtime and bundled plugins.
	# Discover every copy so pruning follows the shipped dependency tree.
	local classic_prebuilds
	while IFS= read -r -d '' classic_prebuilds; do
		find "${classic_prebuilds}" -mindepth 1 -maxdepth 1 -type d \
			! -name "linux-${node_arch}" -exec rm -r {} + || die
		find "${classic_prebuilds}" -type f -name '*.musl.node' -delete || die
	done < <(find usr/lib/chatgpt/resources -type d -path '*/classic-level/prebuilds' -print0)
