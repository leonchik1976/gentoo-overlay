#!/usr/bin/env python3
"""Create pnpm 11 offline registry metadata layouts."""
import json
import sys
...
for name, record in sorted(metadata.items()):
    ...
    for layout in ("v11/metadata", "v11/metadata-full", "v11/metadata-full-filtered"):
        output = cache_root / layout / "registry.npmjs.org" / f"{name}.jsonl"
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(json.dumps({"modified": record["modified"]}) + "\n" + json.dumps(record, sort_keys=True, separators=(",", ":")) + "\n", encoding="utf-8")
for layout in ("v11/metadata", "v11/metadata-full", "v11/metadata-full-filtered"):
    cliui = json.loads((cache_root / layout / "registry.npmjs.org/@isaacs/cliui.jsonl").read_text().splitlines()[1])
