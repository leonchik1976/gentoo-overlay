...
def dependency_specifier(locator):
    # Snapshot aliases are package locators, while registry manifests require
    # the npm: protocol (for example typescript@6.0.2 -> npm:typescript@6.0.2).
...
for key, snapshot in lock.get("snapshots", {}).items():
    ...
    for field in ("dependencies", "optionalDependencies"):
        resolved = {
            dependency: dependency_specifier(value)
...
for name, record in sorted(metadata.items()):
    ...
    for cache_format in ("v11/metadata", "v11/metadata-full", "v11/metadata-full-filtered"):
        output = cache_root / cache_format / "registry.npmjs.org" / f"{name}.jsonl"
        output.parent.mkdir(parents=True, exist_ok=True)
        output.write_text(
            json.dumps({"modified": record["modified"]}) + "\n" + json.dumps(record, sort_keys=True, separators=(",", ":")) + "\n",
            encoding="utf-8",
        )
...
for cache_format in ("v11/metadata", "v11/metadata-full", "v11/metadata-full-filtered"):
    ...
    cohere = json.loads(output.read_text(encoding="utf-8").splitlines()[1])
