...
UADK_COMMIT="267cf34617cc5d6af63191464db40eefcfb78448"
...
SRC_URI+="
	arm64? (
		https://github.com/ceph/uadk/archive/${UADK_COMMIT}.tar.gz
			-> ceph-uadk-${UADK_COMMIT}.tar.gz
	)
"
...
src_prepare() {
	...
	if use arm64; then
		local uadk_src="${WORKDIR}/uadk-${UADK_COMMIT}"

		# https://bugs.gentoo.org/973292
		# Use the UADK source fetched through SRC_URI instead of cloning it.
		eapply "${FILESDIR}/ceph-20.2.1-uadk.patch"

		pushd "${uadk_src}" > /dev/null || die
		eapply "${FILESDIR}/ceph-20.2.1-uadk-const.patch"
		popd > /dev/null || die

		mv "${uadk_src}" "${S}/uadk" || die
	fi
