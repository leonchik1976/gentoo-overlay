...
PATCHES=( "${FILESDIR}/${P}-fix-meson-cargo-build-chain.patch" )
...
src_configure() {
	# Instapaper requires a registered OAuth consumer key/secret compiled
	# into the binary; upstream only ships these via Flathub's private CI
	# secrets and there is no public default pair. Building without your
	# own credentials
	# (https://www.instapaper.com/main/request_oauth_consumer_token) would
	# silently produce a binary that installs fine but can never sign in,
	# so refuse to configure at all rather than ship that. Checked here
	# (immediately before the values are actually consumed by meson)
	# rather than in pkg_pretend, since pkg_pretend runs in an earlier,
	# separate invocation and is not the phase that actually uses these
	# variables. Set CAULDRON_CONSUMER_KEY / CAULDRON_CONSUMER_SECRET in
	# your environment (e.g. /etc/portage/env) before emerging. This is a
	# local overlay convenience, not an upstream-supported mechanism.
