...
# Upstream's worker/setup.py at this tag no longer lists dev-python/six as
# a dependency (dropped upstream since the last ::gentoo version of this
# ebuild); it was intentionally not carried forward here.
...
\"/etc/conf.d/buildbot_worker.myinstance\" at the right location.
...
pkg_config() {
	...
	# ${buildworker_path} via path traversal once interpolated below).
