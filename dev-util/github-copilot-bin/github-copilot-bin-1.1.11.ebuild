...
SRC_URI="
	amd64? (
		https://github.com/github/app/releases/download/v${PV}/GitHub-Copilot-linux-x64.deb
			-> ${P}-amd64.deb
	)
	arm64? (
		https://github.com/github/app/releases/download/v${PV}/GitHub-Copilot-linux-arm64.deb
			-> ${P}-arm64.deb
	)
"
S="${WORKDIR}"
...
# Verified by extracting both the amd64 and arm64 v1.1.11 .deb assets
...
# checked with `readelf -d` on both the amd64 and arm64 v1.1.11 assets,
...
# confirmed via `readelf -Ws` on the v1.1.11 amd64 and arm64 binaries:
...
src_prepare() {
	...
	# Verified independently with `readelf -d` on both v1.1.11 release
