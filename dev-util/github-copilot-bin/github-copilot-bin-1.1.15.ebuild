...
# Verified by extracting both the amd64 and arm64 v1.1.12 .deb assets
...
# checked with `readelf -d` on both the amd64 and arm64 v1.1.12 assets,
...
# confirmed via `readelf -Ws` on the v1.1.12 amd64 and arm64 binaries:
...
src_prepare() {
	...
	# Verified independently with `readelf -d` on both v1.1.12 release
