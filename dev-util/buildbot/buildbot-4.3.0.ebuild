...
# NOTE: ::gentoo removed the whole dev-util/buildbot* family (bug #952430,
# 2026-08-21) for lack of a maintainer and Python 3.13 support. Re-added
# here at the latest upstream release (4.3.0; upstream's own CI only tests
# Python up to 3.13 for this tag, 3.14 support is unreleased/master-only).
# It also requires dev-python/autobahn, which ::gentoo separately removed
# for unrelated quality-trust reasons (see dev-python/autobahn).
...
pkg_config() {
	...
	read instance_name
	# Restrict to a safe charset (no '.', so "." / ".." can't escape
	# ${buildmaster_path} via path traversal once interpolated below).
	[[ "${instance_name}" =~ ^[A-Za-z0-9_-]+$ ]] || \
		die "Invalid instance name: only letters, digits, '_' and '-' are allowed"
