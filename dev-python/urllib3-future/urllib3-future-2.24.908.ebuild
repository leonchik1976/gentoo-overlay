...
python_test() {
	local site_dir="${BUILD_DIR}/install$(python_get_sitedir)/urllib3_future"
	local EPYTEST_IGNORE=(
		# This optional SOCKS adapter requires the upstream socks extra.
		"${site_dir}/contrib/_socks_override.py"
		# WASI platform adapters are not applicable to Gentoo Linux.
		"${site_dir}/contrib/wasi"
	)
	epytest --import-check "${site_dir}"
}
