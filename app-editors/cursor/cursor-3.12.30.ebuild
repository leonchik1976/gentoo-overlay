...
src_install() {
	...
	if use amd64; then
		rm -rf "${CURSOR_HOME}/resources/app/extensions/cursor-agent/dist/claude-agent-sdk/vendor/ripgrep/arm64-linux" || die
	elif use arm64; then
		rm -rf "${CURSOR_HOME}/resources/app/extensions/cursor-agent/dist/claude-agent-sdk/vendor/ripgrep/x64-linux" || die
	fi
