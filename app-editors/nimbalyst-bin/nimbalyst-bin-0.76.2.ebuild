...
# dep (elkjs, unmodified), one dynamically-loaded LGPL-3.0 dep
# (libheif-js), one Python-2.0-licensed transitive dep (argparse, mapped
# to Gentoo's PYTHON license file), and one proprietary exception:
# @anthropic-ai/claude-agent-sdk, whose bundled "claude" CLI binary is (c)
# Anthropic PBC, all rights reserved ("Use is subject to the Legal
# Agreements outlined here: https://code.claude.com/docs/en/legal-and-
# compliance", per that component's own LICENSE.md), under Anthropic's
# Commercial Terms of Service (https://www.anthropic.com/legal/commercial-
# terms). Represented below with Gentoo's stock all-rights-reserved
