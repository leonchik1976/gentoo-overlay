...
# dev-perl/CSS-Inliner is an upstream requirement first introduced between
# 5.0.6 and 5.0.10 (per etc/cpanfile and sbin/rt-test-dependencies). It is
# now packaged in this overlay (dev-perl/CSS-Inliner, plus its transitive
# deps dev-perl/HTML-Query and dev-perl/Badger).
DEPEND="
	acct-group/rt
	acct-user/rt
...
src_configure() {
	...
	# RT autodetects these features from the presence of the "dot" and
	# "gdlib-config" binaries in the build environment, which makes the
	# dependency check non-deterministic. Pin them explicitly: graphviz
	# dependency charts are USE-controlled, GD pie/bar charts are disabled.
	use graphviz && depsconf+=" --with-GRAPHVIZ"
